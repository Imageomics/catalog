...
**`public/config.yaml`**: This is the main file to edit. It contains all configuration options with inline comments explaining each setting:
...
> [!IMPORTANT]
> **Required token**: The weekly tag scan workflow requires a fine-grained access token with **Pull requests: Read and write** permission on the catalog repo. Follow the instructions in [App Authentication](docs/app-authentication.md) to create and install a private Catalog Automation App for token generation.
